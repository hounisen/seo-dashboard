...
const DEFAULT_FORM: FormState = {
  url: '', competitorUrls: '', targetKeyword: '', semanticKeywords: '',
  customKeywords: '', pageTitle: '', metaDescription: '', h1: '',
  bodyContent: '', structuredDataTypes: [],
}
  ...
  return (
    <svg width="110" height="110" viewBox="0 0 110 110">
      ...
      <circle cx="55" cy="55" r={r} fill="none" stroke={color} strokeWidth="8" strokeLinecap="round"
        strokeDasharray={circ} strokeDashoffset={circ - (pct / 100) * circ} transform="rotate(-90 55 55)"
...
function StatusPill({ status }: { status: 'ok' | 'warn' | 'error' }) {
  const map = { ok: { bg: 'bg-emerald-50', text: 'text-emerald-700', dot: 'bg-emerald-400', label: 'Optimised' }, warn: { bg: 'bg-amber-50', text: 'text-amber-700', dot: 'bg-amber-400', label: 'Advarsel' }, error: { bg: 'bg-red-50', text: 'text-red-600', dot: 'bg-red-400', label: 'Fejl' } }
  const s = map[status]
  return <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${s.bg} ${s.text}`}><span className={`w-1.5 h-1.5 rounded-full ${s.dot}`} />{s.label}</span>
}
...
function KwStatusTag({ status }: { status: KeywordStatus }) {
  ...
  return <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold ${map[status]}`}><span className={`w-1.5 h-1.5 rounded-full ${dots[status]}`} />{status}</span>
}

function ProgressBar({ pct }: { pct: number }) {
  return (
    <div className="relative h-4 rounded-full overflow-hidden" style={{ background: 'linear-gradient(90deg, #ff4d6a 0%, #ff8c42 20%, #ffb547 40%, #a8d85e 60%, #2dce89 80%, #e0e4ea 80%)' }}>
      <div className="absolute top-[-5px] w-0 h-0 transition-all duration-1000 ease-out"
        style={{ left: `${Math.min(pct * 0.8 + 1, 79)}%`, borderLeft: '7px solid transparent', borderRight: '7px solid transparent', borderTop: '10px solid #1a1a2e', transform: 'translateX(-50%)' }} />
    </div>
  )
}
...
interface LiveEditorProps {
  scraped: ScrapedSnapshot; form: FormState
  updateForm: (key: keyof FormState, val: string) => void
  setForm: React.Dispatch<React.SetStateAction<FormState>>
  result: SeoResult | null; originalResult: SeoResult | null
}

function LiveEditor({ scraped, form, updateForm, setForm, result, originalResult }: LiveEditorProps) {
  const pct = result?.percentage ?? 0, origPct = originalResult?.percentage ?? 0, delta = pct - origPct
  const hasChanges = scraped.pageTitle !== form.pageTitle || scraped.metaDescription !== form.metaDescription || scraped.h1 !== form.h1 || scraped.bodyContent !== form.bodyContent
  return (
    ...
    <div className="space-y-5">
            ...
            {cannibal.map((r, i) => (
              <div key={i} className="p-4 bg-red-50 rounded-xl border border-red-100">
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-2 text-xs"><span className="w-2 h-2 bg-emerald-400 rounded-full flex-shrink-0"></span><span className="text-gray-500">Vinder:</span><span className="font-mono text-gray-700 truncate">{shortUrl(r.primaryUrl)}</span></div>
                  {r.otherUrls.map((u, j) => <div key={j} className="flex items-center gap-2 text-xs"><span className="w-2 h-2 bg-red-400 rounded-full flex-shrink-0"></span><span className="text-gray-500">Kannibaliserer:</span><span className="font-mono text-gray-600 truncate">{shortUrl(u)}</span></div>)}
                </div>
              </div>
            ))}
          </div>
          <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-800">
            <strong>Anbefaling:</strong> Sæt kanonisk tag på de kannibaliserende sider eller konsolider indholdet og lav 301-redirects.
          </div>
        </div>
      )}
      ...
      {quickWins.length > 0 && (
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 animate-fade-up">
          ...
          <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg text-xs text-blue-800">
            <strong>Anbefaling:</strong> Forbedring fra position 20 til top 10 kan give 5–10× flere klik. Fokus: title, meta og indholdsdybde.
          </div>
        </div>
      )}
      ...
      {dominantUrl && dominantUrl[1] >= 4 && (
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 animate-fade-up">
          ...
          <p className="text-xs text-gray-500 mt-3">Denne side overtager muligvis for mange keywords. Overvej om keyword-ansvaret er korrekt fordelt.</p>
        </div>
      )}
          ...
          <table className="w-full text-sm min-w-[640px]">
            <thead>
              <tr className="border-b border-gray-100">
                {['Keyword', 'Volume', 'Ahrefs pos.', 'GSC pos.', 'Visninger', 'CTR', 'Status'].map(h => <th key={h} className="text-left text-xs font-bold tracking-widest uppercase text-gray-400 pb-3 px-3">{h}</th>)}
              </tr>
            </thead>
            <tbody>
              {data.map((r, i) => {
                  ...
                  <tr key={i} className="border-b border-gray-50 hover:bg-gray-50 transition">
                    ...
                    <td className="py-3 px-3">
                      {r.currPosition && r.posChange
                        ? <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${r.posChange > 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-600'}`}>{r.posChange > 0 ? `▲ +${r.posChange}` : `▼ ${r.posChange}`}</span>
    ...
    <div className="mt-3 rounded-xl border border-gray-100 overflow-hidden">
          ...
          <tbody>
            {show.map((r, i) => (
              <tr key={i} className="border-b border-gray-50 hover:bg-gray-50 transition">
                ...
                <td className={`py-2 px-3 text-right tabular-nums font-bold ${r.position <= 3 ? 'text-emerald-600' : r.position <= 10 ? 'text-amber-600' : 'text-red-500'}`}>
      ...
      {data.length > 5 && (
        <button onClick={() => setExpanded(!expanded)}
          className="w-full py-2 text-xs text-gray-400 hover:text-gray-600 hover:bg-gray-50 transition font-medium">
          {expanded ? '↑ Vis færre' : `↓ Vis alle ${data.length} keywords`}
        </button>
      )}
    </div>
  )
...
function AhrefsPreview({ data }: { data: AhrefsRow[] }) {
  ...
  return (
    <div className="mt-3 rounded-xl border border-gray-100 overflow-hidden">
      {cannibalCount > 0 && (
        <div className="px-3 py-2 bg-red-50 border-b border-red-100 text-xs text-red-600 font-semibold">
          ⚡ {cannibalCount} af {data.length} keywords har kannibalisering
        </div>
      )}
      <div className="overflow-x-auto">
        <table className="w-full text-xs min-w-[340px]">
            ...
            {show.map((r, i) => {
              ...
              return (
                <tr key={i} className={`border-b border-gray-50 hover:bg-gray-50 transition ${isCannibal ? 'bg-red-50/40' : ''}`}>
                  <td className="py-2 px-3 font-medium text-gray-700 truncate max-w-[130px]">
                    <div className="flex items-center gap-1">
                      {isCannibal && <span className="text-red-400 flex-shrink-0">⚡</span>}
                  ...
                  <td className={`py-2 px-3 text-right tabular-nums font-bold ${!pos ? 'text-gray-300' : pos <= 3 ? 'text-emerald-600' : pos <= 10 ? 'text-amber-600' : 'text-red-500'}`}>
                    {pos ? `#${pos}` : '–'}
                  </td>
                  <td className="py-2 px-3 text-right tabular-nums">
                    <span className={`px-1.5 py-0.5 rounded text-xs font-bold ${isCannibal ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-500'}`}>
                      {r.otherUrls.length + (r.primaryUrl ? 1 : 0)}
                    </span>
                  </td>
                </tr>
              )
...
export default function SeoDashboard() {
  ...
  useEffect(() => {
    ...
    setResult(analyzeSeo({ url: form.url, competitorUrls: form.competitorUrls.split('\n').map(s => s.trim()).filter(Boolean), targetKeyword: form.targetKeyword.trim(), semanticKeywords: form.semanticKeywords.split(',').map(s => s.trim()).filter(Boolean), pageTitle: form.pageTitle, metaDescription: form.metaDescription, h1: form.h1, bodyContent: form.bodyContent, structuredDataTypes: form.structuredDataTypes }))
  }, [form])

  useEffect(() => {
    if (!form.targetKeyword || !scraped.pageTitle) return
    setOriginalResult(analyzeSeo({ url: form.url, competitorUrls: [], targetKeyword: form.targetKeyword.trim(), semanticKeywords: form.semanticKeywords.split(',').map(s => s.trim()).filter(Boolean), pageTitle: scraped.pageTitle, metaDescription: scraped.metaDescription, h1: scraped.h1, bodyContent: scraped.bodyContent, structuredDataTypes: form.structuredDataTypes }))
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [form.targetKeyword, form.semanticKeywords, scraped, form.structuredDataTypes])

  const updateForm = useCallback((key: keyof FormState, val: string) => setForm(prev => ({ ...prev, [key]: val })), [])
  ...
  const handleScrape = useCallback(async () => {
    ...
    try {
      ...
      const s = { pageTitle: data.title || '', metaDescription: data.description || '', h1: data.h1 || '', bodyContent: data.bodyContent || '' }
      setScraped(s)
      setForm(prev => ({ ...prev, pageTitle: s.pageTitle || prev.pageTitle, metaDescription: s.metaDescription || prev.metaDescription, h1: s.h1 || prev.h1, bodyContent: s.bodyContent || prev.bodyContent }))
      if (form.targetKeyword) setOriginalResult(analyzeSeo({ url: form.url, competitorUrls: [], targetKeyword: form.targetKeyword.trim(), semanticKeywords: form.semanticKeywords.split(',').map(s2 => s2.trim()).filter(Boolean), ...s, structuredDataTypes: form.structuredDataTypes }))
  ...
  const handleGscUpload = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (p.length >= 4) data.push({ keyword: p[0], impressions: parseInt(p[1]) || 0, clicks: parseInt(p[2]) || 0, position: parseFloat(p[3]) || 0 })
      }
      setGscData(data)
    }
    ...
    reader.onload = async (evt) => {
      try {
        ...
        for (let i = 2; i < raw.length; i++) {
          ...
          rows.push({ keyword: String(r[0]), prevPosition: r[1] != null ? Number(r[1]) : null, currPosition: r[2] != null ? Number(r[2]) : null, posChange: r[3] != null ? Number(r[3]) : null, volume: r[4] != null ? Number(r[4]) : 0, primaryUrl: r[5] ? String(r[5]) : '', otherUrls: otherRaw, clicks: r[7] != null ? Number(r[7]) : 0, impressions: r[8] != null ? Number(r[8]) : 0, ctr: r[9] != null ? Number(r[9]) : 0, gscPosition: r[10] != null ? Number(r[10]) : 0 })
        }
        setAhrefsData(rows)
  ...
  const analyzeCompetitor = useCallback(async () => {
    ...
    if (!urls.length || !form.targetKeyword) return
    setAnalyzingCompetitor(true)
    try {
      const results = await Promise.all(urls.map(async (u) => {
        try {
          const res = await fetch('/api/scrape', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url: u }) })
          const data = await res.json()
          if (!res.ok || data.error) return null
          return { url: u, result: analyzeSeo({ url: u, competitorUrls: [], targetKeyword: form.targetKeyword.trim(), semanticKeywords: form.semanticKeywords.split(',').map(s => s.trim()).filter(Boolean), pageTitle: data.title || '', metaDescription: data.description || '', h1: data.h1 || '', bodyContent: data.bodyContent || '' }) }
        } catch { return null }
      }))
          ...
          {result && (
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2 text-sm">
                ...
                {originalResult && pct !== originalResult.percentage && <span className={`text-xs font-semibold px-1.5 py-0.5 rounded ${pct > originalResult.percentage ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-600'}`}>{pct > originalResult.percentage ? '▲' : '▼'} {Math.abs(pct - originalResult.percentage)}%</span>}
                <span className="text-gray-400 text-xs">SEO Score</span>
              </div>
              <button onClick={() => setSidebarCollapsed(!sidebarCollapsed)} className="px-3 py-1.5 text-xs font-medium text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded-md transition">
          ...
          <div className={`${sidebarCollapsed ? 'hidden' : 'block'} p-6 space-y-6`}>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <input type="url" placeholder="https://www.hounisen.com/nitrilhandsker" value={form.url} onChange={e => updateForm('url', e.target.value)}
                  className="flex-1 px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-400 transition" style={{ fontFamily: 'Inter, monospace' }} />
                <button onClick={handleScrape} disabled={!form.url || scraping}
                  className="px-6 py-3 rounded-xl text-sm font-semibold text-white transition-all disabled:opacity-50 active:scale-95" style={{ background: scraping ? '#94a3b8' : '#4f7fff' }}>
                ...
                <button onClick={() => { if (confirm('Ryd alle felter og start forfra?')) { setForm({ ...DEFAULT_FORM }); setScraped({ pageTitle: '', metaDescription: '', h1: '', bodyContent: '' }); setOriginalResult(null); setGscData([]); setGscFileName(''); setAhrefsData([]); setAhrefsFileName(''); setCompetitorAnalysis([]); setScrapeError(''); setActivePanel('dashboard') } }}
                  className="px-5 py-3 rounded-xl bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-semibold transition-all active:scale-95">🗑️ Ryd alt</button>
              </div>
              {scrapeError && <p className="mt-2 text-xs text-red-500 flex items-center gap-1">⚠ {scrapeError}</p>}
              <div className="mt-4">
                <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide block mb-2">Konkurrent URLs (én pr. linje)</label>
                <textarea rows={2} placeholder="https://www.konkurrent.dk/nitrilhandsker" value={form.competitorUrls} onChange={e => updateForm('competitorUrls', e.target.value)}
                  className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-400 transition resize-none" style={{ fontFamily: 'Inter, monospace' }} />
                {form.competitorUrls && form.targetKeyword && (
                  <button onClick={analyzeCompetitor} disabled={analyzingCompetitor}
                    className="mt-3 px-4 py-2 rounded-lg text-xs font-semibold text-white transition-all disabled:opacity-50" style={{ background: analyzingCompetitor ? '#94a3b8' : '#7c5cff' }}>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" placeholder="f.eks. nitrilhandsker" value={form.targetKeyword} onChange={e => updateForm('targetKeyword', e.target.value)}
                ...
                <div>
                  ...
                  <textarea rows={3} placeholder="pudderfri nitrilhandsker, latexallergi, engangshandsker" value={form.semanticKeywords} onChange={e => updateForm('semanticKeywords', e.target.value)}
                ...
                <div>
                  ...
                  <textarea rows={2} placeholder="køb nitrilhandsker, nitrilhandsker tilbud" value={form.customKeywords} onChange={e => updateForm('customKeywords', e.target.value)}
                    className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-400 transition resize-none" />
                ...
                {/* Structured Data */}
                <div>
                  <label className="text-xs font-semibold text-gray-500 block mb-2">Structured Data / JSON-LD <span className="text-gray-300">(markér hvad siden har)</span></label>
                  ...
                  {form.structuredDataTypes.length > 0 && <p className="text-xs text-emerald-600 mt-2 font-medium">✓ {form.structuredDataTypes.join(', ')} – tæller med i scoren</p>}
                  <p className="text-xs text-gray-400 mt-1.5">Tjek via <a href="https://search.google.com/test/rich-results" target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">Google Rich Results Test</a></p>
                </div>
                ...
                {/* Ahrefs Upload */}
                <div>
                  <label className="text-xs font-semibold text-gray-500 block mb-1.5">Ahrefs + GSC analyse <span className="text-gray-300">(Excel .xlsx)</span></label>
                    ...
                    <>
                      <div className="flex items-center justify-between mt-2">
                        <div className="flex items-center gap-2">
                          ...
                          {ahrefsData.filter(r => r.otherUrls.length > 0).length > 0 && <span className="text-xs bg-red-100 text-red-600 px-1.5 py-0.5 rounded-full font-bold">⚡ {ahrefsData.filter(r => r.otherUrls.length > 0).length} kannibal.</span>}
                        </div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              ...
              <div className="space-y-3 opacity-60 pointer-events-none">
                <div>
                  <label className="text-xs font-semibold text-gray-500 flex items-center justify-between mb-1.5"><span>Title Tag</span><span className="font-normal tabular-nums">{form.pageTitle.length}/65</span></label>
                  <div className="w-full px-4 py-2.5 rounded-xl border border-gray-100 bg-gray-50 text-sm text-gray-500 truncate min-h-[42px]">{form.pageTitle || <span className="italic text-gray-300">tom</span>}</div>
                </div>
                <div>
                  <label className="text-xs font-semibold text-gray-500 flex items-center justify-between mb-1.5"><span>Meta Description</span><span className="font-normal tabular-nums">{form.metaDescription.length}/160</span></label>
                  <div className="w-full px-4 py-2.5 rounded-xl border border-gray-100 bg-gray-50 text-sm text-gray-500 line-clamp-2 min-h-[42px]">{form.metaDescription || <span className="italic text-gray-300">tom</span>}</div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              ...
              <div className="space-y-3 opacity-60 pointer-events-none">
                <div>
                  ...
                  <div className="w-full px-4 py-2.5 rounded-xl border border-gray-100 bg-gray-50 text-sm text-gray-500 truncate min-h-[42px]">{form.h1 || <span className="italic text-gray-300">tom</span>}</div>
                </div>
                <div>
                  <label className="text-xs font-semibold text-gray-500 flex items-center justify-between mb-1.5"><span>Brødtekst</span><span className="font-normal tabular-nums">{result?.wordCount ?? form.bodyContent.split(/\s+/).filter(Boolean).length} ord</span></label>
                  <div className="w-full px-4 py-2.5 rounded-xl border border-gray-100 bg-gray-50 text-sm text-gray-500 line-clamp-3 min-h-[60px]">{form.bodyContent || <span className="italic text-gray-300">tom</span>}</div>
                </div>
              </div>
            </div>
          </div>
                ...
                {!result ? (
                  <div className="bg-white rounded-2xl p-12 text-center shadow-sm border border-gray-100">
                    {(scraped.pageTitle || scraped.bodyContent) && !form.targetKeyword ? (
                      <><div className="text-3xl mb-3">✅</div><p className="text-gray-700 text-sm font-semibold mb-1">Side scraped – mangler kun target keyword</p><p className="text-gray-400 text-xs">Udfyld <span className="font-semibold text-blue-500">Target keyword</span> i venstre side</p></>
                    ) : (
                      <><p className="text-gray-400 text-sm">Udfyld target keyword og scrape URL for at se analysen.</p><p className="text-gray-500 text-xs mt-2">Resultaterne vises live her mens du ændrer indstillinger ←</p></>
                    )}
                  </div>
                ) : (
                  <>
                          ...
                          <div className="flex items-start justify-between mb-3">
                            <div>
                              <div className="flex items-baseline gap-2">
                                ...
                                {originalResult && pct !== originalResult.percentage && <span className={`text-sm font-bold px-2 py-0.5 rounded-full ${pct > originalResult.percentage ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-600'}`}>{pct > originalResult.percentage ? '▲' : '▼'} {Math.abs(pct - originalResult.percentage)}% ift. original</span>}
                              </div>
                              <div className={`text-sm font-semibold mt-0.5 ${pct >= 70 ? 'text-emerald-600' : pct >= 45 ? 'text-amber-600' : 'text-red-500'}`}>{pct >= 70 ? 'God' : pct >= 45 ? 'Middel – kan forbedres' : 'Kritisk – kræver handling'}</div>
                            </div>
                            <div className="text-right text-sm text-gray-400">Nå <span className="font-semibold text-gray-700">70%</span> for at matche<br />gennemsnitlige konkurrenter</div>
                          </div>
                          <ProgressBar pct={pct} />
                          ...
                          <table className="w-full text-sm min-w-[600px]">
                            ...
                            <tbody>
                              <tr className="border-b border-gray-100">
                                ...
                                {competitorAnalysis.map((c,i)=><td key={i} className="py-3 px-3 text-center text-gray-700 font-semibold">{c.result.wordCount}</td>)}
                              </tr>
                              <tr className="border-b border-gray-100">
                                ...
                                {competitorAnalysis.map((c,i)=><td key={i} className="py-3 px-3 text-center text-gray-700 font-semibold tabular-nums">{c.result.targetKeywordCount}×</td>)}
                              </tr>
                              {(result?.semanticKeywordsTotal||0)>0&&<tr className="border-b border-gray-100">
                                ...
                                {competitorAnalysis.map((c,i)=><td key={i} className="py-3 px-3 text-center text-gray-700 font-semibold tabular-nums">{c.result.semanticKeywordsCovered}/{c.result.semanticKeywordsTotal}</td>)}
                              </tr>}
                              <tr className="border-b border-gray-100">
                                ...
                                {competitorAnalysis.map((c,i)=><td key={i} className="py-3 px-3 text-center text-gray-700 font-semibold">{c.result.readabilityScore}</td>)}
                              </tr>
                              <tr>
                                ...
                                {competitorAnalysis.map((c,i)=><td key={i} className="py-3 px-3 text-center text-gray-700 text-2xl font-bold">{c.result.percentage}%</td>)}
                              </tr>
                            </tbody>
                          </table>
                        </div>
                        ...
                        {result.recommendations.map(rec => (
                          <div key={rec.id}>
                            <button onClick={() => setOpenRec(openRec === rec.id ? null : rec.id)}
                              ...
                              <div><div className="text-xs font-bold tracking-wide uppercase text-gray-400 mb-1.5">{rec.label}</div><StatusPill status={rec.status} /></div>
                              <span className="text-gray-300 text-lg font-light ml-2 flex-shrink-0">›</span>
                            </button>
                            {openRec === rec.id && <div className="mt-1 p-4 bg-white border border-gray-100 rounded-xl text-sm text-gray-600 leading-relaxed" dangerouslySetInnerHTML={{ __html: rec.detail.replace(/\n/g, '<br/>') }} />}
                          </div>
                        ))}
                      </div>
                    </div>
                      ...
                      <div className="flex flex-wrap gap-2 mb-6">
                        {result.keywords.map(k => (
                          <span key={k.keyword} className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-transform hover:scale-105 ${k.status==='God'?'bg-emerald-50 text-emerald-700':k.status==='Optimer'?'bg-amber-50 text-amber-700 border border-amber-200':'bg-red-50 text-red-600 border border-red-200'}`}>
                            {k.keyword}{k.status==='God'&&<span className="text-emerald-500">✓</span>}{k.currentCount>0&&k.status!=='God'&&<span className="bg-black/10 rounded-full px-1.5">{k.currentCount}×</span>}
                          </span>
                        ))}
                      </div>
                      <div className="overflow-x-auto -mx-2">
                        <table className="w-full text-sm min-w-[480px]">
                          <thead><tr className="border-b border-gray-100">{['Keyword','Status','Anbefalet','Nuværende'].map(h=><th key={h} className="text-left text-xs font-bold tracking-widest uppercase text-gray-400 pb-3 px-3">{h}</th>)}</tr></thead>
                          <tbody>
                            {result.keywords.map(k=>(
                    ...
                    {gscData.length > 0 && (
                      <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 animate-fade-up-3">
                        ...
                        <div className="overflow-x-auto -mx-2">
                          <table className="w-full text-sm min-w-[480px]">
                            <thead><tr className="border-b border-gray-100">{['Keyword','Clicks','Impressions','Position','På siden?'].map(h=><th key={h} className="text-left text-xs font-bold tracking-widest uppercase text-gray-400 pb-3 px-3">{h}</th>)}</tr></thead>
                            <tbody>
                              {gscData.slice(0,10).map((row,i)=>{
                                ...
                                return <tr key={i} className="border-b border-gray-50 hover:bg-gray-50 transition">
                      ...
                      <div className="space-y-3">
                        {result.quickWins.map((w,i)=>(
                          <div key={i} className="flex items-start gap-3 p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition border border-transparent hover:border-gray-200">
                            ...
                            <div><div className="font-semibold text-sm text-gray-800 mb-0.5">{w.title}</div><div className="text-xs text-gray-500 leading-relaxed">{w.detail}</div></div>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    {/* Keyword sections */}
                    {(() => {
                      const optKws=result.keywords.filter(k=>k.status==='Optimer')
                      const misKws=result.keywords.filter(k=>k.status==='Mangler')
                      const custKws=form.customKeywords.split(',').map(k=>k.trim()).filter(Boolean)
                      return (<>
                        {optKws.length>0&&<div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 animate-fade-up-5">
                          <p className="text-xs font-bold tracking-widest uppercase text-gray-400 mb-4">Use These Keywords More</p>
                          <div className="flex flex-wrap gap-2">{optKws.map((kw,i)=><span key={i} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold bg-amber-50 text-amber-700 border border-amber-200">{kw.keyword}<span className="text-amber-500">({kw.currentCount})</span></span>)}</div>
                        </div>}
                        {misKws.length>0&&<div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 animate-fade-up-6">
                          <p className="text-xs font-bold tracking-widest uppercase text-gray-400 mb-4">Consider Semantic Rich Keywords</p>
                          <div className="flex flex-wrap gap-2">{misKws.map((kw,i)=><span key={i} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold bg-red-50 text-red-600 border border-red-200">{kw.keyword}</span>)}</div>
                        </div>}
                        {custKws.length>0&&<div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 animate-fade-up-7">
                          <p className="text-xs font-bold tracking-widest uppercase text-gray-400 mb-4">Custom Keywords</p>
                          <div className="flex flex-wrap gap-2">{custKws.map((kw,i)=>{
                            const txt=[form.pageTitle,form.metaDescription,form.h1,form.bodyContent].join(' ').toLowerCase()
                            const found=(txt.match(new RegExp(kw.toLowerCase().trim().replace(/\s+/g,'\\s*'),'g'))||[]).length>0
                            return <span key={i} className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold ${found?'bg-emerald-50 text-emerald-700':'bg-red-50 text-red-600 border border-red-200'}`}>{kw}{found&&<span className="text-emerald-500">✓</span>}</span>
                          })}</div>
                        </div>}
                      </>)
                    })()}
