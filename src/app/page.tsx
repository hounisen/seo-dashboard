...
export default function SeoDashboard() {
  ...
  const handleGscUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (isExcel) {
      // Parse Excel – handles GSC export format with Danish or English headers
      ...
      reader.onload = async (evt) => {
        try {
          ...
          // Detect column indices from header row (row 0)
          // Handles both Danish ("Mest anvendte forespørgsler", "Klik", "Eksponeringer", "Placering")
          // and English ("Top queries", "Clicks", "Impressions", "Position")
          const headers = (raw[0] as string[]).map(h => String(h ?? '').toLowerCase().trim())
          const colKeyword = headers.findIndex(h => h.includes('forespørg') || h.includes('quer') || h.includes('keyword') || h.includes('søgeord'))
          const colClicks  = headers.findIndex(h => h === 'klik' || h.includes('click'))
          const colImpr    = headers.findIndex(h => h.includes('eksponering') || h.includes('impression'))
          const colPos     = headers.findIndex(h => h.includes('placering') || h.includes('position'))

          if (colKeyword === -1) return // unrecognised format

          const data: GscData[] = []
          for (let i = 1; i < raw.length; i++) {
            const r = raw[i]
            const kw = r[colKeyword]
            if (!kw || typeof kw !== 'string') continue
            data.push({
              keyword: kw,
              clicks:      colClicks !== -1 && r[colClicks]  != null ? Number(r[colClicks])  : 0,
              impressions: colImpr   !== -1 && r[colImpr]    != null ? Number(r[colImpr])    : 0,
              position:    colPos    !== -1 && r[colPos]     != null ? Number(r[colPos])     : 0,
            })
          }
          setGscData(data)
        } catch (err) {
          console.error('GSC Excel parse error:', err)
        }
      }
      reader.readAsArrayBuffer(file)
    } else {
      // Parse CSV (original behaviour)
      const reader = new FileReader()
      reader.onload = (evt) => {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          if (parts.length >= 4) {
            data.push({ keyword: parts[0], impressions: parseInt(parts[1]) || 0, clicks: parseInt(parts[2]) || 0, position: parseFloat(parts[3]) || 0 })
          }
        }
        setGscData(data)
      }
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      {gscFileName || 'Upload GSC eksport (.xlsx fra GSC eller .csv)'}
                    </label>
                  </div>
                  {gscData.length > 0 && (
                    <>
